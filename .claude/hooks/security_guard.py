...
CRITICAL_PATTERNS = [
    ...
    {
        "regex": re.compile(r"(password|secret|api_?key|token|private_?key)\s*[=:]\s*['\"][\w\-/+=]{16,}['\"]", re.I),
    ...
    # SQL injection via f-strings (limited to 500 chars to prevent ReDoS)
    {
        "regex": re.compile(r'f["\'].{0,500}?(SELECT|INSERT|UPDATE|DELETE).{0,200}?(FROM|INTO|WHERE|SET).{0,200}?\{', re.I | re.DOTALL),
        "message": "CRITICAL: SQL injection risk (f-string in query)",
        "severity": "critical"
    },
    # Command injection via subprocess with variables
    {
        "regex": re.compile(r'subprocess\.(run|call|Popen|check_output)\s*\([^)]*\{', re.I),
    ...
    {
        "regex": re.compile(r'\b(open|read|write|unlink|remove)\s*\([^)]{0,200}?f["\'][^"\']*\{', re.I),
...
def main():
    ...
    try:
        ...
        try:
            sys.stderr.write(f"HOOK_LOG_ERROR: {json.dumps({
                'timestamp': datetime.now().isoformat(),
                'hook': 'security_guard',
