...
- **Final Task**: ALWAYS assign `review-orchestrator` as the absolute last task
- **Task Name**: "Final quality review and approval"
- **Blocking**: NO workflow can complete without review-orchestrator approval
...
This ensures 100% code coverage with comprehensive review coordination (config-security + code-quality + edge-cases) before any merge or completion.
...
**Code Review**: git diff analysis → review-orchestrator → (config-security + code-quality + edge-case parallel) → consolidated findings
