...
clone_repository() {
    ...
    if ! validate_ssh_access "${repo_url}"; then
        error "Cannot access repository ${repo_url}"
        error "Ensure SSH key is configured for GitHub"
        return 1
    fi
...
# PostgreSQL Database
DATABASE_URL=postgresql://trivance_user:trivance_password@localhost:5432/trivance_${repo_name}
EOF
